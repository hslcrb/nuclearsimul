 ...
 */
const NAV_BOOL_PROPS = ['enabled', 'bidirectional', 'avoidance_enabled'];
function sanitizeBoolProps(properties) {
    if (!properties)
        return properties;
...
export function registerNavigationTools(server, bridge) {
    ...
    server.registerTool('setup_navigation_region', {
        ...
        const a = args;
        a.properties = sanitizeBoolProps(a.properties);
    ...
    server.registerTool('find_navigation_path', {
        ...
        const startArr = args.start || [];
        const endArr = args.end || [];
        ...
        return callGodot(bridge, 'navigation/find_path', args);
    });
