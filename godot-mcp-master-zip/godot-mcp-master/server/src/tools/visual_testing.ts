...
export function registerVisualTestingTools(server: McpServer, bridge: GodotBridge): void {
  ...
  server.registerTool(
    ...
    async (args) => callGodot(bridge, 'delete_screenshot', args as Record<string, unknown>),
  );

  // 8. delete_visual_recording
  server.registerTool(
    'delete_visual_recording',
    {
    ...
    async (args) => callGodot(bridge, 'delete_visual_recording', args as Record<string, unknown>),
  );

  // 9. clear_visual_diff_report
  server.registerTool(
    'clear_visual_diff_report',
    {
